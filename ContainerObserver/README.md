...
This project demonstrates how to both build a plugin and then deploy FabricObserver with the plugin onboard directly from the plugin project's build output folder. You could always simply build the plugin and copy the dll to the /PackageRoot/Config/Data folder in your FabricObserver solution and then deploy FO from there. In that case, you will need to modify FO's Settings.xml and ApplicationManifest.xml to configure your plugin. 
...
Copy-ServiceFabricApplicationPackage -ApplicationPackagePath $path -CompressPackage -ApplicationPackagePathInImageStore FabricObserverV3111 -TimeoutSec 1800
Register-ServiceFabricApplicationType -ApplicationPathInImageStore FabricObserverV3111
